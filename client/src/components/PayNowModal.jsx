...
import { X, ShieldCheck, CheckCircle, Loader, Upload, AlertCircle } from 'lucide-react';
import toast from 'react-hot-toast';
import API from '../services/api';

/**
 * PayNowModal - Handles UPI QR payment + captures UTR proof
 */
export default function PayNowModal({ isOpen, onClose, order, user, onPaymentSuccess }) {
  ...
  const handleConfirmPayment = async (e) => {
    e.preventDefault();
    if (!utr.trim()) {
      toast.error('UTR / Transaction ID is mandatory');
      return;
    }

    setLoading(true);
    try {
      // Send proof to backend for admin verification
      await API.post(`/payments/order/${order._id}/mark-paid`, {
        utr: utr.trim(),
        ...
        >
          ...
          {step === 'qr' && (
            <div className="p-5 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="font-mono text-sm font-semibold bg-gray-100 px-3 py-1.5 rounded-lg inline-block mt-1">9975882125@okbizaxis</p>
              </div>
              ...
              <button onClick={() => setStep('form')}
                className="w-full py-3 border-2 border-green-600 text-green-700 rounded-xl text-sm font-bold hover:bg-green-50 transition flex items-center justify-center gap-2">
                <CheckCircle size={16} /> I Have Paid — Submit Proof
              </button>
            </div>
          )}
          ...
          {step === 'form' && (
            <form onSubmit={handleConfirmPayment} className="p-5 space-y-4">
              <div className="bg-yellow-50 text-yellow-800 p-3 rounded-lg flex items-start gap-2 text-xs">
                <AlertCircle size={14} className="mt-0.5 flex-shrink-0" />
                <p>Please enter your UPI transaction details. We will process your invoice once verified.</p>
              </div>
              ...
              <div>
                ...
                <input type="text" required value={utr} onChange={e => setUtr(e.target.value)}
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none text-sm font-mono" />
              </div>
              ...
              <div>
                ...
                <input type="text" value={referenceNumber} onChange={e => setReferenceNumber(e.target.value)}
                  placeholder="App order ref / external ID"
              ...
              <div>
                ...
                <div 
                  ...
                  {screenshotBase64 ? (
                    <img src={screenshotBase64} alt="Proof preview" className="h-20 object-contain rounded-md" />
                  ) : (
                    <>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" disabled={loading}
                  className="flex-[2] py-3 bg-green-600 text-white rounded-xl font-bold text-sm hover:bg-green-700 transition flex items-center justify-center gap-2 disabled:opacity-60">
          ...
          {step === 'success' && (
            <div className="p-8 text-center space-y-4">
              ...
              <h4 className="text-xl font-bold text-gray-800">Verification Pending!</h4>
              <p className="text-sm text-gray-500">Your proof for order #{orderId} has been submitted.</p>
              <p className="text-xs text-gray-400">Once Admin confirms your UPI transfer, your invoice will be available to download!</p>
              <button onClick={handleClose}
                className="w-full py-3 bg-green-600 text-white rounded-xl font-bold text-sm hover:bg-green-700 transition mt-4">
                Close Modal
              </button>
            </div>
          )}
